...
function Collect-SystemInfo {
    ...
    Invoke-With-Timeout -ScriptBlock {
        ...
        $reportContent = @"
<h3>Relatório de Manutenção do Sistema</h3>
...
function Collect-Profiles {
    ...
    Invoke-With-Timeout -ScriptBlock {
        ...
        if ($profiles.Count -gt 0) {
            ...
            if (![string]::IsNullOrEmpty($ReportPath)) {
                Add-Content -Path $ReportPath -Value "<br><h3>Perfis de Usuário</h3>$profileDetails" -Encoding UTF8
            }
        } else {
            Add-Content -Path $ReportPath -Value "<br><h3>Perfis de Usuário</h3>Nenhum perfil encontrado." -Encoding UTF8
        }
    } -TimeoutSeconds 300 -ReportPath $ReportPath
}
    ...
    Invoke-With-Timeout -ScriptBlock {
        ...
        try {
            ...
            if ($errors.Count -gt 0) {
                ...
                if (![string]::IsNullOrEmpty($ReportPath)) {
                    Add-Content -Path $ReportPath -Value "<br><h3>Integridade do Sistema (Últimos 10 Eventos)</h3>$formattedErrors" -Encoding UTF8
                }
            } else {
                Add-Content -Path $ReportPath -Value "<br><h3>Integridade do Sistema</h3>Nenhum erro crítico encontrado." -Encoding UTF8
            }
        } catch {
            Add-Content -Path $ReportPath -Value "<br><h3>Integridade do Sistema</h3>Erro ao verificar integridade do sistema." -Encoding UTF8
        }
    } -TimeoutSeconds 300 -ReportPath $ReportPath
}
...
function Check-Security {
    ...
    Invoke-With-Timeout -ScriptBlock {
        ...
        try {
            ...
            if (![string]::IsNullOrEmpty($ReportPath)) {
                Add-Content -Path $ReportPath -Value "<br><h3>Verificação de Segurança</h3>$securityDetails" -Encoding UTF8
            }
        } catch {
            Add-Content -Path $ReportPath -Value "<br><h3>Verificação de Segurança</h3>Erro ao verificar segurança do sistema." -Encoding UTF8
        }
    } -TimeoutSeconds 300 -ReportPath $ReportPath
}
...
function Collect-Printers {
    ...
    Invoke-With-Timeout -ScriptBlock {
        ...
        if ($printers.Count -gt 0) {
            ...
            if (![string]::IsNullOrEmpty($ReportPath)) {
                Add-Content -Path $ReportPath -Value "<br><h3>Impressoras Instaladas</h3>$($printerDetails -join "<br>")" -Encoding UTF8
            }
        } else {
            Add-Content -Path $ReportPath -Value "<br><h3>Impressoras Instaladas</h3>Nenhuma impressora instalada." -Encoding UTF8
        }
    } -TimeoutSeconds 300 -ReportPath $ReportPath
}
...
function Update-Windows {
    ...
    Invoke-With-Timeout -ScriptBlock {
        ...
        try {
            $hotfixes = Get-HotFix
            if ($hotfixes) {
                Add-Content -Path $ReportPath -Value "<br><h3>Atualizações do Windows</h3>Atualizações listadas com sucesso." -Encoding UTF8
            } else {
                Add-Content -Path $ReportPath -Value "<br><h3>Atualizações do Windows</h3>Nenhuma atualização pendente." -Encoding UTF8
            }
        } catch {
            Add-Content -Path $ReportPath -Value "<br><h3>Atualizações do Windows</h3>Erro ao verificar ou instalar atualizações do Windows." -Encoding UTF8
        }
    } -TimeoutSeconds 300 -ReportPath $ReportPath
}
...
function Update-Software {
    ...
    Invoke-With-Timeout -ScriptBlock {
        ...
        try {
            ...
            if ($exitCode -eq 0) {
                Add-Content -Path $ReportPath -Value "<br><h3>Atualizações de Software</h3>Atualização de software concluída com sucesso." -Encoding UTF8
            } else {
                Add-Content -Path $ReportPath -Value "<br><h3>Atualizações de Software</h3>Erro ao atualizar software. Código de saída: $exitCode" -Encoding UTF8
            }
        } catch {
            Add-Content -Path $ReportPath -Value "<br><h3>Atualizações de Software</h3>Erro ao atualizar software." -Encoding UTF8
        }
    } -TimeoutSeconds 300 -ReportPath $ReportPath
}
...
function Clean-TemporaryFiles {
    ...
    Invoke-With-Timeout -ScriptBlock {
        ...
        try {
            ...
            Add-Content -Path $ReportPath -Value "<br><h3>Limpeza de Arquivos Temporários</h3>Limpeza realizada com sucesso." -Encoding UTF8
        } catch {
            Add-Content -Path $ReportPath -Value "<br><h3>Limpeza de Arquivos Temporários</h3>Erro ao realizar limpeza." -Encoding UTF8
        }
    } -TimeoutSeconds 300 -ReportPath $ReportPath
}
    ...
    foreach ($disk in $disks) {
        if ($disk.MediaType -eq "HDD") {
            ...
            if ($userResponse -eq "s") {
                ...
                if ($chkdskOutput -match "Não é possível bloquear a unidade atual") {
                    ...
                    if ($scheduleResponse -eq "s") {
                        chkdsk C: /f /r /x
                        Add-Content -Path $ReportPath -Value "<br><h3>Manutenção de Disco</h3>Verificação agendada para a próxima inicialização." -Encoding UTF8
                    } else {
                        Add-Content -Path $ReportPath -Value "<br><h3>Manutenção de Disco</h3>Verificação de disco não agendada." -Encoding UTF8
                    }
                } else {
                    Add-Content -Path $ReportPath -Value "<br><h3>Manutenção de Disco</h3>Verificação de disco realizada com sucesso." -Encoding UTF8
                }
                defrag C: /O
            } else {
                Add-Content -Path $ReportPath -Value "<br><h3>Manutenção de Disco</h3>Manutenção de HD não realizada." -Encoding UTF8
            }
            ...
            if ($userResponse -eq "s") {
                ...
                Add-Content -Path $ReportPath -Value "<br><h3>Manutenção de Disco</h3>Manutenção de SSD realizada." -Encoding UTF8
            } else {
                Add-Content -Path $ReportPath -Value "<br><h3>Manutenção de Disco</h3>Manutenção de SSD não realizada." -Encoding UTF8
            }
        }
    }
}
...
function Repair-Windows {
    ...
    foreach ($command in $commands) {
        ...
        if ($userResponse -eq "s") {
                ...
                try {
                    ...
                    if ($output -match "não é possível bloquear a unidade atual" -or $output -match "O volume está em uso") {
                        ...
                        if ($scheduleResponse -eq "s") {
                            ...
                            Add-Content -Path $ReportPath -Value "<br><h3>Comando: $($command.Name)</h3>Descrição: $($command.Description)<br>Verificação agendada para o próximo reinício.<br>" -Encoding UTF8
                            Write-Host "Verificação agendada para o próximo reinício." -ForegroundColor Green
                        } else {
                            Add-Content -Path $ReportPath -Value "<br><h3>Comando: $($command.Name)</h3>Verificação não foi agendada pelo usuário.<br>" -Encoding UTF8
                            Write-Host "Verificação não agendada." -ForegroundColor Yellow
                        }
                    } else {
                        Add-Content -Path $ReportPath -Value "<br><h3>Comando: $($command.Name)</h3>Descrição: $($command.Description)<br>Saída:<br>$output<br>" -Encoding UTF8
                    }
                } catch {
                    Write-Host "Erro ao executar o comando: $($command.Name)" -ForegroundColor Red
                    Add-Content -Path $ReportPath -Value "<br><h3>Comando: $($command.Name)</h3>Erro ao executar comando: $_<br>" -Encoding UTF8
                }
            } else {
                try {
            ...
            Add-Content -Path $ReportPath -Value "<br><h3>Comando: $($command.Name)</h3>Comando não executado pelo usuário.<br>" -Encoding UTF8
        }
    }
...
function Organize-Report {
    ...
    if (![string]::IsNullOrEmpty($ReportPath)) {
        Add-Content -Path $ReportPath -Value "<br><h3>Tempo Total de Execução</h3>$formattedTime" -Encoding UTF8
    }
...
$report# Define a política de execução para permitir o script
Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope LocalMachine -Force
